...
class UploadProvider extends ServiceProvider
{
    ...
    public function move($from, $to): array
    {
        ...
        // 检测是否存在原文件
        $rawPath = $fileInfo['dirname'] . '/';
        $rawFile = $fileInfo['filename'] . '_raw.' . $fileInfo['extension'];
        if ($uploadObject->exists($rawPath . $rawFile)) {
            $uploadObject->move($rawPath . $rawFile, $to . '/' . $rawFile);
        }
        // 检测是否存在附件
